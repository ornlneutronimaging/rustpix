...
pub struct SoAAbsClustering {
    config: SoAAbsConfig,
}

pub struct SoAAbsState {
    buckets: Vec<SoABucket>,
...
impl SoAAbsClustering {
    pub fn new(config: SoAAbsConfig) -> Self {
        Self { config }
    }
    ...
    fn get_bucket(&self, state: &mut SoAAbsState) -> usize {
        if let Some(idx) = state.free_indices.pop() {
            idx
        } else {
            let idx = state.buckets.len();
            state.buckets.push(SoABucket::new());
            idx
        }
    }

    fn close_bucket(&self, bidx: usize, state: &mut SoAAbsState, batch: &mut HitBatch) {
